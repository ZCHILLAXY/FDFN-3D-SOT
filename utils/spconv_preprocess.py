...
def spconv_process_pointcloud(point_cloud, template=False, is_testset=False):
    if not is_testset:
...
def spconv_process_project(point_cloud, template=False, is_testset=False):
    if not is_testset:
        max_number_of_voxel = 16000
    else:
        max_number_of_voxel = 40000
    try:
        ...
        from spconv.utils import VoxelGenerator
    if template:
        rgb_range = [0, 0, 0, cfg.TEMPLATE_INPUT_WIDTH, cfg.TEMPLATE_INPUT_WIDTH, 80]
        voxel_size = np.array([1, 1, 80], dtype=np.float32)
    else:
        rgb_range = [0, 0, 0, cfg.SCENE_INPUT_WIDTH, cfg.SCENE_INPUT_WIDTH, 80]
        voxel_size = np.array([1, 1, 80], dtype=np.float32)
    max_point_number = cfg.VOXEL_POINT_COUNT

    voxel_generator = VoxelGenerator(
        voxel_size=voxel_size,
        point_cloud_range=rgb_range,
        max_num_points=max_point_number,
        max_voxels=max_number_of_voxel
    )
    voxel_output = voxel_generator.generate(point_cloud)
